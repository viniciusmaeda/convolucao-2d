...
if (__name__ == "__main__"):
  ...
  img = ax.imshow(imagemTensor.numpy(), cmap='gray')
  # oculta as legendas dos eixos
  ax.set_xticks([])
  ax.set_yticks([])
  ...
  for i in range(1, imagemTensor.size(0) - 1):
    for j in range(1, imagemTensor.size(1) - 1):
      ...
      plt.pause(0.0005)
      # Atualizar para a próxima imagem
      img.set_data(matrizResultado.numpy())
      plt.title(nucleos[opcao]['nome'] + '\n' + f'Pixel: ({i}, {j})')
      plt.draw()
